...
%base_nav_path = '/media/results/nomad/2026-01-28-11-05' ;
base_nav_path = '/media/results/vint/2026-02-01-11-23' ;
traj_path = sprintf('%s/traj', base_nav_path) ;

tmp = split(base_nav_path,'/') ;
navtime_id = tmp{end} ;
%nav_summary_file = sprintf('base_nav')
%readlines() ;
...
exportgraphics(gca, 'vint_res.png', 'Resolution', 300);
exportgraphics(gca, 'vint_res.pdf', 'Resolution', 300);
...
legend('Topology nodes','Start node', 'End node', 'Node to node direction',  'Location','southwest');
hold off;
