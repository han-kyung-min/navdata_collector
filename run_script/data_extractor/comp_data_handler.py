...
class comp_data_handler():
    def __init__(self, name, bagfile_path, **kwargs):
        ...
        self.odom_f = nav_msgs.msg.Odometry
        ...
        self.extraction_path = '%s/decoded_bags'%(self.bagfile_path )
    ...
    def runExtractor(self ):
        ...
        for bagfile in pbar:
            ...
            for topic, msg, t in self.bag.read_messages(topics=[self.metadata_topic]):
                ...
                out_depth_file = '%s/depth%05d.png' %(self.out_depth_paths[bagfile_idx], frameidx)
                ...
                self.writeDepthData(out_depth_file, msg.depth)
                self.writeImageData(out_rgb_file, msg.rgb)
    ...
    def writeImageData(self, out_img_file, img_msg):
        #print(img_msg.data)
        try:
            cv_image = self.bridge.imgmsg_to_cv2(img_msg, "bgr8")
            cv2.imwrite(out_img_file, cv_image)
        except CvBridgeError as e:
            print(e)

    def writeDepthData(self, out_depth_file, depth_msg):
        #print(img_msg.data)
        try:
            cv_image = self.bridge.imgmsg_to_cv2(depth_msg, "16UC1")
            print(np.max(cv_image))
            cv_image = np.float32(cv_image) / 65535 * 10000
            ...
            cv2.imwrite(out_depth_file, cv_image)
        except CvBridgeError as e:
            print(e)
