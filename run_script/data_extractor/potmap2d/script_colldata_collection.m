...
topomap_dir = '/home/hankm/python_ws/viznav/depth-nav/deployment/topomaps/round10-1' ;
...
sync_metadata_dir = '/media/results/navdata_collector/collision_data/processed/coll_2025-08-21-18-33/bag_2025-08-21-18-33-42/synced' ;
...
for data_idx = 1 : num_data
    ...
    % draw next 5 future sgs
    sgs_corrected_px = zeros(num_node,2) ;
    for ii=1:num_node
        next_sg_idx = min(ii, num_node) ;
        bHsg = inv( m2Hb_curr ) * topo_m1Hb(:,:,next_sg_idx) ; 
        sgs_corrected_px(ii,:) = bHsg(1:2,4)' / resolution ;
    end

    sg_cand_idx = find( sgs_corrected_px(:,1) > 0 ) ;
    sgs_corrected_px = sgs_corrected_px(sg_cand_idx,:) ;
    Q = sqrt (sum( sgs_corrected_px.*sgs_corrected_px, 2 ) )  ;
    minidx = find(Q == min(Q) )  ;
    sg_corrected_px = sgs_corrected_px(minidx,:) ;
    ...
    imshow(rgb_sg)  ;
    title('SG (corrected) RGB [{\color{green}●}]')
    ...
    plot( sg_corrected_px(1) + rx, sg_corrected_px(2) + ry, 'go', 'MarkerSize', 12, 'MarkerFaceColor', 'g' ) ;
    if (joy_on)
        %rectangle('position', [rx - roi_back_px, ry - roi_front_px / 2, roi_front_px, roi_front_px], 'EdgeColor', [1,0,1] ) ;
        plot( rx, ry, 'cs', 'MarkerSize', 15, 'MarkerFaceColor', 'c' ) ;
    else
    ...
    plot( rx + sg_px(1), rx + sg_px(2),  'mo', 'MarkerSize', 8 , 'MarkerFaceColor', 'm') ;
    ...
    lgd = legend('Pred Waypts', 'SG(corrected)', 'Robot', 'SG(pred)', 'Location', 'NW')
    ...
    P0 = [rx, ry] ; 
    P2 = [sg_corrected_px(1), sg_corrected_px(2)] + P0 ;
    ...
    context_idxs = [ data_idx-4*ws:ws:data_idx ] ;
    ...
    out_waypts_m = out_waypts; 
    out_waypts_m(:,1:2) =  out_waypts_m(:,1:2)* resolution ;
    fid = fopen( sprintf('%s/corrected_waypoints_m.txt', out_dir), 'w' ) ;
    fprintf( fid, '%6.4f %6.4f %6.4f %6.4f \n', out_waypts_m' ) ; 
    fclose(fid) ;

    % (5) save pred waypoints
    pred_waypts_m = reshape( waypt_line(9:end), 4, nc)' ;
