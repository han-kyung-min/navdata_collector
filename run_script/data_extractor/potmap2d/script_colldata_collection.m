...
for data_idx = 1 : num_data
    ...
    sg_cand_idx = find( sgs_xyzq_corrected_px(:,1) > 0 ) ; % anything in front of the robot
    sgs_xyzq_corrected_px = sgs_xyzq_corrected_px(sg_cand_idx,:) ;
    Q = sqrt (sum( sgs_xyzq_corrected_px(:,1:2).*sgs_xyzq_corrected_px(:,1:2), 2 ) )  ;
    minidx = find(Q == min(Q) )  ;

    new_sg_idx = sg_cand_idx(minidx(1)) ;
    new_sg_xyzq_px = sgs_xyzq_corrected_px(minidx, :) ;
    ...
    imshow(rgb_sg)  ;
    title('SG (corrected) RGB [{\color{blue}●}]')
    ...
    plot( new_sg_xyzq_px(1) + rx, new_sg_xyzq_px(2) + ry, 'bo', 'MarkerSize', 12, 'MarkerFaceColor', 'b' ) ;
    if (joy_on)
        %rectangle('position', [rx - roi_back_px, ry - roi_front_px / 2, roi_front_px, roi_front_px], 'EdgeColor', [1,0,1] ) ;
        plot( rx, ry, 'gs', 'MarkerSize', 15, 'MarkerFaceColor', 'g' ) ;
    else
