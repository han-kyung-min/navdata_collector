...
[topo_odom_raw, topo_odom_xy, oHb ] = load_pose_data(topo_odom_file) ; 
[topo_m2b_raw, topo_m2b_xy, mHb ] = load_pose_data(topo_m2b_file) ; 
[topo_m2o_raw, topo_m2o_xy, mHo ] = load_pose_data(topo_m2o_file) ; 


sync_metadata_dir = '/media/results/navdata_collector/collision_data/processed/coll_2025-08-21-11-14/bag_2025-08-21-11-14-04/synced' ;
...
for data_idx = 1:num_data
    ...
    sg_px = subgoals(data_idx, 5:6) / resolution ;  % relative subgoal 
    ...
    plot( rx + sg_px(1), rx + sg_px(2),  'ms', 'MarkerSize',12 , 'MarkerFaceColor', 'm') ;
    if (joy_on)
    ...
    if (gx <0 | gx > map_size_px | gy < 0 | gy > map_size_px)
        continue;  % if the pred waypoint is OK. Don't bother
    end
    ...
    quat = quats(context_idxs, :) ;  % qx qy qz qw
    nc = length(context_idxs) ;
    wHr = zeros(4,4, nc) ;

    for qidx=1:nc
        q = quat(qidx, [end, 1:end-1]) ;  % qw, qx, qy, qz
        wHr(:,:,qidx) = quat_to_htm( q ) ;
        didx = context_idxs(qidx) ;
        wHr(1:2,4,qidx) = [ px(didx), py(didx) ] ;
    end
