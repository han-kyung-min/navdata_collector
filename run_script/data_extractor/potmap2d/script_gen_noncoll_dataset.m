...
out_folder = '/media/mydata/former_datasets/colldata/non_colldata-1K' ;
num_data = length(dataset) ;
begin_offset = 200 ; %num_dataset ;

for idx=1:1000 %num_data
    fprintf('\rProcessing <%d>th data ', idx)
    didx = idx + begin_offset ;
    dataset_path = sprintf('%s/data%05d', out_folder, didx) ;
