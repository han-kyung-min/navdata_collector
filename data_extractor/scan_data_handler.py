...
class scan_data_handler():
    def __init__(self, name, bagfile_path, **kwargs):
        ...
        self.bagfile_path   = bagfile_path  # kwargs['navdata_collector']['bagfile_path']
        self.extraction_path       = '%s/decoded_bags' % bagfile_path #kwargs['bagfile_decoder']['outpath']
        ...
        print("extraction path: %s"%self.extraction_path)
    ...
    def runExtractor(self ):
        ...
        for bagfile in pbar:
            ...
            for topic, msg, t in self.bag.read_messages(topics=[self.metadata_topic]):
                ...
                out_scan_file = '%s/scan%05d.txt' %(self.out_scan_paths[bagfile_idx], frameidx)
                out_map_file  = '%s/map%05d.png' %(self.out_map_paths[bagfile_idx], frameidx)
                ...
                self.writeScanData(out_scan_file, msg.scan)
